...
public class WaveFunctionCollapse : MonoBehaviour
{
    ...
    public void InitializeWaveFunction()
    {
        ...
        for (int x = 0, y = 0; x < size.x; x++)
        {
            for (int z = 0; z < size.y; z++)
            {
                ...
                {
#if UNITY_EDITOR
                    GameObject block = Instantiate(allProtoPrefab, pos, Quaternion.identity);
                    block.transform.SetParent(this.transform);
#endif
                }
                Cell cell = this.transform.GetChild(y).gameObject.GetComponent<Cell>();
                cell.coords = new Vector2(x, z);
                cell.possiblePrototypes = new List<Prototype>(prototypes);
                cells.Add(cell);
    ...
    private void CollapseAt(Cell cell)
    {
        ...
        // Spawn the actual road model
        GameObject finalPrefab = Instantiate(finalPrototype.prefab, cell.transform);
        finalPrefab.transform.localPosition = Vector3.zero;
        finalPrefab.transform.localRotation = Quaternion.Euler(0, finalPrototype.meshRotation * 90f, 0);

        // HIDE THE ORIGINAL PLACEHOLDER (The stadium/empty box)
        // This stops the stacking look
        if (cell.GetComponent<MeshRenderer>()) cell.GetComponent<MeshRenderer>().enabled = false;
...
public class BuildingRandomizer : MonoBehaviour
{
    ...
    {
        // Example: Change building color randomly
